...
class SessionHandler
{
  constructor(room, peerToPeer, roomName, callstats) {
    ...
    this._room.localParticipant.on('trackDisabled', (track) => {
      if (track.kind === 'audio') {
        this.sendFabricEvent(this._callstats.fabricEvent.audioMute);
      }
      if (track.kind == 'video') {
        this.sendFabricEvent(this._callstats.fabricEvent.videoPause);
      }
    });

    this._room.localParticipant.on('trackEnabled', (track) => {
      if (track.kind === 'audio') {
        this.sendFabricEvent(this._callstats.fabricEvent.audioUnmute);
      }
      if (track.kind == 'video') {
        this.sendFabricEvent(this._callstats.fabricEvent.videoResume);
      }
    });

    this._room.on('disconnected', (room, cause) => {
      this.sendFabricEvent(this._callstats.fabricEvent.fabricTerminated);
  ...
  sendFabricEvent(fabricEvent) {
    logger.debug('sendFabricEvent() [fabricEvent:%s]', fabricEvent);
